...
__global__
void kernel_convsp(float* locs, float* data, float* density, float* neighborlist, 
	...
	float* dweight, int num_blocks)
{
    ...
    for (int i = index; i < N*batch_size*num_blocks; i += stride)
    {
    	...
    	compute_kernel_cells(locs, data, density, neighborlist, weight, bias, batch_size, N, 
    		nchannels, ndims, nneighbors, nkernels, ncells, radius, kernel_size, dilation, 
    		out, b, n, start, start + block_size, ddata, dweight);
    }
}
int cuda_convsp(float* locs, float* data, float* density, float* neighborlist, 
	float* weight, float* bias, 
	int batch_size, int N, int nchannels, int ndims, int nneighbors, int nkernels, int ncells, 
	float radius, float* kernel_size, float* dilation, float* out, float* ddata,
	float* dweight, cudaStream_t stream)
{
	...
	kernel_convsp<<<blocks, threads, 0, stream>>>(locs, data, density, neighborlist, 
		weight, bias,
		batch_size, N, nchannels, ndims, nneighbors, nkernels, ncells, radius, kernel_size, 
