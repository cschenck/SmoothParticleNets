...
int cpu_convsp(float* locs, float* data, float* density, float* neighborlist,
    float* weight, float* bias, 
    int batch_size, int N, int nchannels, int ndims, int nneighbors, 
    ...
    THFloatTensor* dilation_t, THFloatTensor* out_t)
{
    ...
    return cpu_convsp(locs, data, density, neighborlist, weight, bias, batch_size, N, 
    ...
    float* dweight)
{
    ...
    for(b = 0; b < batch_size; ++b)
    {
        for(n = 0; n < N; ++n)
        {
            compute_kernel_cells(locs, data, density, neighborlist, weight, bias, 
                batch_size, N, 
                nchannels, ndims, nneighbors, 
                nkernels, ncells, radius, kernel_size, dilation, 
