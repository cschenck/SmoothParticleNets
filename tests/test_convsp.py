...
def eval_convsp(cuda=False):
    ...
    weights = np.random.rand(NKERNELS, NCHANNELS, np.prod(
    ...
    for use_qlocs in (True, False):
        ...
        if use_qlocs:
            qlocs_t = torch.autograd.Variable(
                use_cuda(torch.FloatTensor(qlocs)), requires_grad=True)
        else:
            qlocs_t = None
        data_t = torch.autograd.Variable(
            use_cuda(torch.FloatTensor(data)), requires_grad=True)
        ...
        locs_t, data_t, idxs_t, neighbors_t = coll(
            locs_t, data_t, (qlocs_t if use_qlocs else None))
        ...
        for kernel_fn in spn.KERNEL_NAMES:
            ...
            np.testing.assert_array_almost_equal(
                pred_t.data.numpy(), ground_truth, decimal=3)
            ...
            assert gradcheck(func_analytical,
                             ((lt, dt, wt, bt, qt)
                              if use_qlocs else (lt, dt, wt, bt,)),
                             eps=1e-4, atol=1e-3, rtol=1e-1, func_numerical=func_numerical, use_double=True)
